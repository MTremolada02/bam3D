...
void Runner::qname_stats(Bam_record_vector &group) {
    ...
    while (begin < group.size()) {
        ...
        for (std::size_t j = begin; j < end; ++j) {
            ...
            if (flag & BAM_FREAD1) {
                if (!(flag & BAM_FSECONDARY)) r1_side.push_back(j);
                ++mapped_r1;
            ...
            if (flag & BAM_FREAD2) {
                if (!(flag & BAM_FSECONDARY)) r2_side.push_back(j);
                ++mapped_r2;
...
/*        if (mapped_r1 == 0) a = Maptype::N;
        ...
        if (mapped_r2 == 0) b = Maptype::N;
        else if ((mapped_r2 == 1 && group[primary_r2]->core.qual < 1) || primary_r2 == (std::size_t)-1 || secondary_r2 > 0)
            b = Maptype::M;
        else
            b = Maptype::U;
*/
...
if (mapped_r2 == 0)
    ...
    b = Maptype::U;
        ...
        if ((r1_side.size()+r2_side.size()) >= 3) {
            ...
            // --- identifica lato chimerico
            if (r1_side.size() == 2) {
                inner = r1_side[0];
                outer = r1_side[1];
                R1_chim = true;
                other = r2_side.empty() ? (std::size_t)-1 : r2_side[0];
            } else {
                inner = r2_side[0];
                outer = r2_side[1];
                R2_chim = true;
                other = r1_side.empty() ? (std::size_t)-1 : r1_side[0];
            }
  ...
  //                  if (R1_chim) a = Maptype::M;
    //                else         b = Maptype::M;
      //          } else {
                    ...
                    bool rev_i  = group[left]->core.flag & BAM_FREVERSE;
                    bool rev_ot = group[other]->core.flag & BAM_FREVERSE;

                    bool cis = (group[left]->core.tid == group[other]->core.tid);

                    bool facing =
                        ((!rev_i && rev_ot && group[left]->core.pos <= group[other]->core.pos) ||
                         ( rev_i && !rev_ot && group[other]->core.pos <= group[left]->core.pos));

                    bool distance =
                        (llabs(group[left]->core.pos - group[other]->core.pos) <= 2000);
